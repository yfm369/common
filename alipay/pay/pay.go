...
func buildBizContent(charge *Charge) string {
	...
	mc["subject"] = charge.Describe            //订单标题
	mc["out_trade_no"] = charge.TradeNum       //商户订单号
	mc["product_code"] = "QUICK_MSECURITY_PAY" //销售产品码
	mc["total_amount"] = fmt.Sprintf("%.2f",
		float64(charge.MoneyFee)/float64(100)) //订单总金额
